...
const translations = {
  ...
  en: {
    ...
    btn_quarterly: "Quarterly Performance",
...
let favoriteSymbols = []; // Watchlist of starred stock symbols
let searchActiveIndex = -1; // Keyboard navigation index within search dropdown
...
function loadConfig() {
  try {
    ...
    const savedTheme = localStorage.getItem("tplus_theme");
    if (savedTheme) {
      currentTheme = savedTheme;
    }

    const savedFavorites = localStorage.getItem("tplus_favorites");
    if (savedFavorites) {
      favoriteSymbols = JSON.parse(savedFavorites);
    }
  } catch (e) {
    // localStorage may be unavailable (private browsing) or data corrupted; fall back to defaults
    console.error("Failed to load saved configuration, using defaults.", e);
  }
...
function saveConfig() {
  try {
    ...
    localStorage.setItem("tplus_theme", currentTheme);
    localStorage.setItem("tplus_favorites", JSON.stringify(favoriteSymbols));
  } catch (e) {
...
function toggleTheme() {
  ...
  // Re-render chart so its colors/gridlines match the new theme
  if (activeTab === "history5") {
    renderHistoryChart();
  }
}

// Toggle a stock symbol in/out of the favorites watchlist
function toggleFavorite(symbol) {
  const idx = favoriteSymbols.indexOf(symbol);
  if (idx === -1) {
    favoriteSymbols.push(symbol);
  } else {
    favoriteSymbols.splice(idx, 1);
  }
  saveConfig();
  updateUI();
}
...
function setupEventListeners() {
  ...
  searchInput.addEventListener("input", (e) => {
    ...
    // Debounce: wait a beat after typing stops before filtering, avoids
    // re-rendering the dropdown on every single keystroke for large lists.
  ...
  // Keyboard navigation through the search results dropdown
  searchInput.addEventListener("keydown", (e) => {
    const items = Array.from(dropdown.querySelectorAll("li[data-symbol]"));
  ...
  // Favorite/watchlist star on the company header
...
// Apply visual highlight to the currently keyboard-selected dropdown item
function highlightSearchItem(items) {
  items.forEach((item, i) => {
    item.classList.toggle("active-item", i === searchActiveIndex);
...
function renderSearchResults(results) {
  ...
  results.forEach(stock => {
    ...
    starSpan.className = `result-star ${favoriteSymbols.includes(stock.symbol) ? "active" : ""}`;
    starSpan.innerHTML = "★";
    starSpan.setAttribute("aria-label", currentLang === "ar" ? "إضافة/إزالة من المفضلة" : "Add/remove from favorites");
    starSpan.addEventListener("click", (e) => {
      e.stopPropagation();
      toggleFavorite(stock.symbol);
      starSpan.classList.toggle("active");
    });
...
function updateUI() {
  ...
  document.getElementById("tasi-val").textContent = marketSummary.tasi.value.toLocaleString(currentLang === 'ar' ? 'ar-SA' : 'en-US', {minimumFractionDigits: 2});
  ...
  const formattedPrice = stock.price.toLocaleString(currentLang === 'ar' ? 'ar-SA' : 'en-US', {minimumFractionDigits: 2});
  ...
  // Favorite/watchlist star state on the company header
  const favoriteBtn = document.getElementById("favorite-btn");
  if (favoriteBtn) {
    const isFav = favoriteSymbols.includes(currentCompanyId);
    favoriteBtn.classList.toggle("active", isFav);
    favoriteBtn.setAttribute("aria-pressed", String(isFav));
    favoriteBtn.setAttribute("title", isFav
      ? (currentLang === "ar" ? "إزالة من المفضلة" : "Remove from favorites")
      : (currentLang === "ar" ? "إضافة إلى المفضلة" : "Add to favorites"));
  }
  renderFavoritesList();
...
// Render the quick-access favorites/watchlist list in the sidebar
function renderFavoritesList() {
  ...
  if (favoriteSymbols.length === 0) {
    ...
    empty.textContent = currentLang === "ar"
      ? "لا توجد أسهم في المفضلة بعد"
  ...
  favoriteSymbols.forEach(symbol => {
    ...
    chip.type = "button";
    chip.className = `favorite-chip ${symbol === currentCompanyId ? "active" : ""}`;
    chip.innerHTML = `<span class="favorite-chip-symbol">${stock.symbol}</span><span class="favorite-chip-name">${currentLang === "ar" ? stock.nameAr : stock.nameEn}</span>`;
    chip.addEventListener("click", () => loadStock(symbol));
...
function renderMetricsGrid(stock, dict) {
  ...
  metricsConfig.forEach(metricKey => {
    ...
    if (metricKey === "eps") {
      unit = dict.unit_sar;
      formattedVal = val.toLocaleString(currentLang === 'ar' ? 'ar-SA' : 'en-US', {minimumFractionDigits: 2});
      ...
      formattedVal = val.toLocaleString(currentLang === 'ar' ? 'ar-SA' : 'en-US') + unit;
      unit = ""; // embedded in text
    } else if (metricKey === "freeCashFlow") {
      unit = dict.unit_billion;
      formattedVal = val.toLocaleString(currentLang === 'ar' ? 'ar-SA' : 'en-US');
    }
...
function renderHistoryTable(stock, dict) {
  ...
  metricsMap.forEach(item => {
    ...
    values.forEach(val => {
      const td = document.createElement("td");
      td.textContent = val.toLocaleString(currentLang === 'ar' ? 'ar-SA' : 'en-US', {minimumFractionDigits: 1});
      row.appendChild(td);
    });
...
function renderDividendsSection(stock, dict) {
  ...
  stock.dividends.quarterly.forEach(item => {
    ...
    tdAmt.textContent = `${item.amount.toLocaleString(currentLang === 'ar' ? 'ar-SA' : 'en-US', {minimumFractionDigits: 2})} ${dict.unit_sar}`;
...
function renderHistoryChart() {
  ...
  } else if (selectedMetric === "quarterly") {
    const q = stock.quarterlyFinancials;
    chartData = {
      labels: q.quarters,
      datasets: [
        {
          ...
          label: currentLang === "ar" ? "الإيرادات (مليار ر.س)" : "Revenues (Billion SAR)",
          data: q.revenues,
          backgroundColor: "rgba(0, 135, 90, 0.65)",
          borderRadius: 6,
          yAxisID: 'y'
        },
        {
...
function simulatePriceUpdate() {
  ...
  Object.keys(stockData).forEach(symbol => {
    ...
    stock.changePercent = previousClose !== 0
      ? parseFloat(((stock.change / previousClose) * 100).toFixed(2))
      : 0;
  ...
  marketSummary.tasi.changePercent = marketSummary.tasi.value !== 0
    ? parseFloat(((marketSummary.tasi.change / marketSummary.tasi.value) * 100).toFixed(2))
    : 0;
